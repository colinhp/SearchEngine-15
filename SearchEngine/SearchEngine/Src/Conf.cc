...
void extractDataToMap(const char *buf, guoshubo::Conf::ConfMap &confMap)
{
	char key[27] = {0};
	char word[1024] = {0};
	//0. 提取数据
...
//	std::cout << "word = " << word << std::endl; //test
	...
	//1. 将key-word放入map中
	std::pair<guoshubo::Conf::ConfMap::iterator, bool> ret = confMap.insert(std::make_pair(std::string(key), std::string(word)));
	if(!ret.second)
	{
...
void readConf(const char *confpath, guoshubo::Conf::ConfMap &confMap)
{
	//0. 打开配置文件
	FILE *fp = guoshubo::openFileRead(confpath);
	...
	{
	//1.2 将配置信息分解，并填入map中
		guoshubo::extractDataToMap(linebuf, confMap);		
	}
	//bug:line free
}
...
{
	guoshubo::readConf(confpath, confMap_);
}
