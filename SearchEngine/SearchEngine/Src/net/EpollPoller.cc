...
{
	ssize_t checknum = guoshubo::readpeek(peerfd);
	if(0 == checknum)
		return true;
	else
		return false;	
...
EpollPoller::EpollPoller(int listenfd)
	:listenfd_(listenfd),
	 isLooping_(false),
	 epfd_(guoshubo::CreateEpoll())
{
	guoshubo::AddEpollRead(epfd_, listenfd_.getfd());
}
...
void EpollPoller::handleConnection()
{
	...
	guoshubo::AddEpollRead(epfd_, peerfd);
...
void EpollPoller::handleMessage(int peerfd)
{
	...
	//1. 判断连接是否已断
	bool close = guoshubo::ifClose(peerfd);
	if(close)
	{
		myconn->handleClose();

		guoshubo::DelEpollRead(epfd_, peerfd);
		delete myconn;
