...
static guoshubo::ThreadPool *pthreadpool = NULL;
...
int main(int argc, char *argv[])
{
	...
	guoshubo::Conf myconf(argv[1]);
//	guoshubo::Conf myconf("/home/fiona/PROJECT@BUAA/c++/SearchEngine/Conf/server.conf");
	...
	guoshubo::ThreadPool::IndexMap indexMap = myconf.createIndexMap();
	guoshubo::ThreadPool::OffsetMap offsetMap = myconf.createOffset();

	guoshubo::Cache mycache(myconf);

	//2.1 线程池
	guoshubo::ThreadPool thrpool(10, 10, mycache, pagestart, indexMap, offsetMap, mycut);
	pthreadpool = &thrpool;	

	//2.2 定时器
	guoshubo::TimerThread mytimer(std::bind(&guoshubo::ThreadPool::UpdateCache, pthreadpool), 60, 120);//第一次60s，后面间隔120s	

	//2.3 服务器-客户端
	guoshubo::TcpServer myser(pIp, port, 10);
...
void myMessage(guoshubo::TcpConnection *pTcpConn)
{
	...
	guoshubo::SearchTask *mytask = new guoshubo::SearchTask(pTcpConn, pthreadpool->pagestart_, pthreadpool->indexMap_, pthreadpool->offsetMap_, pthreadpool->mycut_);
